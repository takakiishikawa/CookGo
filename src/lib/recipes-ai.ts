...
- main_ingredient_tag は必須。次の 7 種から 1 つだけ選ぶ:
  - "魚" : 魚介類(白身/青魚/エビ/イカ/タコ/貝など)が主役
  - "鳥" : 鶏肉・鴨肉など鳥類が主役
  - "豚" : 豚肉が主役
  - "牛" : 牛肉が主役
  - "麺" : 麺類(うどん/そば/ラーメン/パスタ/ビーフン等)が主役
  - "スープ" : スープ・汁物が主役(ポタージュ/味噌汁/具だくさんスープ等。麺が主役の汁麺は「麺」)
...
- ingredient_info は ingredients と同数(=各食材につき必ず1件)。各食材の魅力を素人にもわかりやすく面白く伝える「食材図鑑」を作るつもりで書くこと。各オブジェクトのフィールドは:
  - name: 対応する ingredients[*].name と完全一致
  - category: ingredients[*].category と同じ値
  - origin: 名前の由来や歴史的トリビア(1〜2文)。語源・伝来エピソード・思わず「へぇ」と言いたくなる小話を入れる
  - composition: 何でできているか・どう作られているか(2〜3文)。専門用語(例:「糊化」「変性」)を避け、身近な比喩で書く
  - taste_profile: 感じる味と、なぜそう感じるかの仕組み(1〜2文)。舌のどこで何を感じる・口の中でどう広がる、を書くと臨場感が出る
  - pairings: 相性のいい食材 1〜3 個。各 { food, reason } で reason は化学的・食感的になぜ合うかを端的に1文
  - alternatives: 代替品 1〜3 個。ホーチミンで入手しやすい順。各 { name, reason }
  - nutrition_tags: 食材単位での栄養特徴 0〜3 個(プレーン日本語の短いラベル)
  - 口調: 親しみのある語り口で固くなりすぎない。「〜なんです」OK。ただし冗長な前置きは入れない。`;
